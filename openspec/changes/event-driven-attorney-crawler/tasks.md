...
**Test:** Unit tests — records serialize/deserialize correctly via Jackson round-trip. No Kafka needed.
...
**Test:** Integration test with embedded Kafka (spring-kafka-test).
- Start app context with embedded broker
...
**Test:** Integration test with embedded Kafka + Testcontainers Postgres.
- Produce a `CrawlUrl` to `urls_to_crawl`
- Verify a `RawPage` appears on `raw_pages` with `status=200` and non-empty HTML
...
2. Jackson: parse JSON → navigate to `attorneys_master_list.data` (array of 1,100 objects)
3. Each JSON object has: `id`, `name`, `surName`, `officeLocations[]`, `offices[]`, `photo`, `practiceAreas[]`, `url`
4. Map each to `AttorneyProfile` (listing-level — no bio, education, bar, memberships, socials)
5. For the first `crawler.max-detail-pages` entries (default: 160), produce `CrawlUrl` with `depth=1`, `pageType=DETAIL`, full URL = `base-url` + `url` field
...
**Jsoup selectors** (see design.md Section 7 for full detail):
...
All selectors return empty string / empty list when element is absent. No NPE risk.
...
**Test:**
- **Unit tests for parsers:** Feed saved HTML fixtures to each parser. For the listing page, save a snapshot of the full page HTML. For detail pages, save 2 snapshots (e.g., John Morgan — rich profile, Blake Lange — minimal profile). Assert extracted fields match expected values. No Kafka, no HTTP.
- **Integration test:** Produce a `RawPage` with fixture HTML to `raw_pages` → verify `AttorneyProfile` on `parsed_items` and `CrawlUrl` messages on `urls_to_crawl`.
...
- `podman compose up -d`
- `./gradlew bootRun`
- Observe logs: scheduler seeds 1 URL → fetcher fetches listing → parser extracts JSON + produces detail URLs → fetcher fetches detail pages → parser extracts profiles → storage persists
- Query Postgres to verify ~1,100 listing-level + ~160 detail-enriched attorney profiles

**Done when:** Full pipeline runs end-to-end. Modulith structure verified. Attorneys in the database.
